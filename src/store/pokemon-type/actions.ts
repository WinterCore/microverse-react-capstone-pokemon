...
export type ActionUnion =
  | FetchPokemonType
  | FetchPokemonTypeSuccess
  | FetchPokemonTypeError;

export const fetch = (id: number) => async (
  dispatch: Dispatch<ActionUnion>,
): Promise<void> => {
  dispatch(
    createAction<typeof FETCH_POKEMON_TYPE, undefined>(
      FETCH_POKEMON_TYPE,
      undefined,
    ),
  );
  try {
    if (id === -1) {
      const { data }: AxiosResponse<GetPokemonsResponse> = await Api(
        GET_POKEMONS(),
      );
      const formatted: PokemonType<NamedApiResource> = {
        id: -1,
        name: 'All',
        pokemon: data.results.map(({ name, url }) => ({
          pokemon: { name, id: extractIdFromUrl('/pokemon', url) },
        })),
      };
      dispatch(
        createAction<
          typeof FETCH_POKEMON_TYPE_SUCCESS,
        PokemonType<NamedApiResource>
        >(FETCH_POKEMON_TYPE_SUCCESS, formatted),
      );
    } else {
      const { data }: AxiosResponse<GetPokemonTypeResponse> = await Api(
        GET_TYPE(id),
      );
      const formatted: PokemonType<NamedApiResource> = {
        ...data,
        pokemon: data.pokemon.map(({ pokemon: { name, url } }) => ({
          pokemon: { name, id: extractIdFromUrl('/pokemon', url) },
        })),
      };
    ...
    dispatch(
      createAction<typeof FETCH_POKEMON_TYPE_ERROR, string>(
        FETCH_POKEMON_TYPE_ERROR,
        'Something happened',
      ),
    );
  }
