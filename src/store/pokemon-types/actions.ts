...
export const fetch = () => async (
  ...
  dispatch(
    createAction<typeof FETCH_POKEMON_TYPES, undefined>(
      FETCH_POKEMON_TYPES,
      undefined,
    ),
  );
  try {
    const { data }: AxiosResponse<GetPokemonTypesResponse> = await Api(
      GET_TYPES(),
    );
    const formatted: PaginatedResource<NamedApiResource[]> = {
      ...data,
      results: data.results.map(({ url, name }) => ({
        name,
    ...
    dispatch(
      createAction<
        typeof FETCH_POKEMON_TYPES_SUCCESS,
      PaginatedResource<NamedApiResource[]>
      >(FETCH_POKEMON_TYPES_SUCCESS, formatted),
    );
  } catch (e) {
    dispatch(
      createAction<typeof FETCH_POKEMON_TYPES_ERROR, string>(
