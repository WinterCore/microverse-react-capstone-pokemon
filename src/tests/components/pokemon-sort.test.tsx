...
describe('PokemonSort Component', () => {
    it('Changes the order of the rendered pokemons', async () => {
        mockedAxios.mockImplementationOnce(() => Promise.resolve({ data: data.paginatedResponse(data.pokemonsResponse) }));

        render(<div><PokemonSort /><PokemonGrid /></div>);

        fireEvent.change(screen.getByTestId('sortSelect'), { target: { value: SortFilter.Desc } });
        ...
        await waitFor(() => {
            ...
            for (let i = 0; i < items.length; i += 1) {
                const item = items[i];
                expect(grid.childNodes[i].childNodes[0].textContent).toEqual(capitalize(item.name));
            }
        });
    });
});
