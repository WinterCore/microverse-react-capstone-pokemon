...
export const pokemonTypesResource: NamedApiResource[] = (
  pokemonTypesResponse.map(
    x => ({ name: x.name, id: extractIdFromUrl('/type', x.url) }),
  )
);
...
export const pokemonTypeResource: PokemonType<NamedApiResource> = {
  ...pokemonTypeResponse,
  pokemon: pokemonTypeResponse.pokemon.map(x => ({
    pokemon: {
      id: extractIdFromUrl('/pokemon', x.pokemon.url),
      name: x.pokemon.name,
    },
  })),
};
...
export const pokemonsResource: NamedApiResource[] = pokemonsResponse.map(
  x => ({ id: extractIdFromUrl('/pokemon', x.url), name: x.name }),
);
